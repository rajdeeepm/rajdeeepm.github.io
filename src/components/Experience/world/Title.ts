...
export default class Title {
  ...
  positionItem() {
    ...
    // Check if this is a menu item or a project title
    const menuItems = ['i AM RM', 'Works', 'About', 'Contact']
    const isMenuItem = menuItems.includes(this.settings.text)
    ...
    if (isMenuItem) {
      ...
      const geometry = this.mesh.geometry as MSDFTextGeometry
      const textWidth = (geometry.layout?.width || 0) * this.scale
      this.mesh.position.x = -textWidth / 2
    }
